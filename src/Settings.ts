...
import {
    cleanUpStatusEmojiSettings,
    renderStatusEmojiSettings,
} from './settings/StatusEmojiSettings';
...
export class SettingsManager {
    ...
    constructUI(contentEl: HTMLElement, heading: string, local: boolean) {
        ...
        new Setting(contentEl)
            .setName(t('Expand lists to full width in list view'))
            .then((setting) => {
                let toggleComponent: ToggleComponent;
        ...
        contentEl.createEl('h4', { text: t('Status Emojis') });

        new Setting(contentEl)
            .setDesc(
                t('Define custom status emojis that can be applied to cards in specific lanes')
            )
            .then((setting) => {
                const [value] = this.getSetting('status-emojis', local);

                const statusEmojis = (value || []) as StatusEmoji[];

                renderStatusEmojiSettings(setting.settingEl, statusEmojis, (emojis) => {
                    this.applySettingsUpdate({
                        'status-emojis': {
                            $set: emojis,
                        },
                ...
                this.cleanupFns.push(() => {
                    if (setting.settingEl) {
                        cleanUpStatusEmojiSettings(setting.settingEl);
                    }
                });
            });
        ...
        new Setting(contentEl).setName(t('Date display format')).then((setting) => {
            setting.addMomentFormat((mf) => {
                setting.descEl.appendChild(
                    createFragment((frag) => {
                        frag.appendText(
                            t('This format will be used when displaying dates in Kanban cards.')
                        );
                        frag.createEl('br');
                        frag.appendText(t('For more syntax, refer to') + ' ');
                        frag.createEl(
    ...
    cleanUp() {
        ...
        // Get document elements correctly
        const container = this.win.document.body;

        cleanUpDateSettings(container);
        cleanUpTagSettings(container);
        cleanUpTagSortSettings(container);
        cleanupMetadataSettings(container);
        cleanUpStatusEmojiSettings(container);
    }
}
