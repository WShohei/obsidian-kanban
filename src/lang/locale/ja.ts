...
const lang: Partial<Lang> = {
    ...
    // settings/StatusEmojiSettings.tsx
    'Status Emojis': 'ステータス絵文字',
    'Define custom status emojis that can be applied to cards in specific lanes':
        'リストに配置されたカードに適用できるカスタムステータス絵文字を定義する',
    'Add Status Emoji': 'ステータス絵文字を追加',
    Emoji: '絵文字',
    Name: '名前',
    'Status name': 'ステータス名',
    'Mark as completion status': '完了ステータスとしてマーク',
    'If enabled, this status will be considered as a completion status':
        '有効にすると、このステータスは完了ステータスとして扱われます',
    'In Progress': '進行中',

    // components/Lane/LaneSettings.tsx
    'Apply status to cards in this list': 'このリストのカードに適用するステータス',
    None: 'なし',
    'Completion (✅)': '完了 (✅)',
};
