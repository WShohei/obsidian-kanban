...
export class StateManager {
    ...
    compileSettings(suppliedSettings?: KanbanSettings) {
        ...
        this.compiledSettings = {
            ...
            'date-trigger':
                this.getSettingRaw('date-trigger', suppliedSettings) || defaultDateTrigger,
            'inline-metadata-position':
                this.getSettingRaw('inline-metadata-position', suppliedSettings) ||
                defaultMetadataPosition,
            'time-format': timeFormat,
            'time-trigger':
                this.getSettingRaw('time-trigger', suppliedSettings) || defaultTimeTrigger,
    ...
    async archiveCompletedCards() {
        ...
        const statusEmojis = this.getSetting('status-emojis') || [];

        // List of status emoji IDs set as completion
        const completionStatusIds = statusEmojis
            .filter((emoji) => emoji.isCompletion)
            .map((emoji) => emoji.id);
            ...
            return update(lane, {
                children: {
                    $set: lane.children.filter((item) => {
                        // Standard completion check (checkbox state)
                        const isComplete =
                            item.data.checked && item.data.checkChar === getTaskStatusDone();

                        // Status set by the lane
                        const laneSetCompletion =
                            lane.data.shouldMarkItemsComplete ||
                            (lane.data.shouldApplyStatus &&
                                completionStatusIds.includes(lane.data.shouldApplyStatus));

                        // Condition for archiving
                        const shouldArchive = isComplete || laneSetCompletion;
