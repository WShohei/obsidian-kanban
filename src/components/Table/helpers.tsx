                    ...
                    itemInlineMetadata.forEach((m) => {
                        if (!inlineMetadata.has(m.key)) {
                            ...
                            if (!metadataLabels.has(m.key)) {
                                if (taskFields.has(m.key))
                                    metadataLabels.set(m.key, lableToName(m.key));
                                else {
...
export function useTableColumns(boardData: Board, stateManager: StateManager) {
    ...
    const state = useMemo(
        () => ({ sorting, globalFilter: search?.query }),
                                    ...
                                    if (sorted === null) {
                                        ...
                                        const aSortOrder =
                                            tagSortOrder?.findIndex((sort) =>
                                                tagsA.includes(sort.tag)
                                            ) ?? -1;
                                        const bSortOrder =
                                            tagSortOrder?.findIndex((sort) =>
                                                tagsB.includes(sort.tag)
                                            ) ?? -1;
        ...
        for (const key of inlineMetadata) {
            columns.push(
                columnHelper.accessor(
                    (row) => {
                        const data = row.item.data.metadata.inlineMetadata?.find(
                            (m) => m.key === key
                        );
                        if (data) return data;
                        return null;
                        ...
                        sortingFn: (a, b, id) => {
                            ...
                            if (sorted === null) {
                                if (id === 'tags') {
                                    ...
                                    const aSortOrder =
                                        tagSortOrder?.findIndex((sort) =>
                                            valA.value.includes(sort.tag)
                                        ) ?? -1;
                                    const bSortOrder =
                                        tagSortOrder?.findIndex((sort) =>
                                            valB.value.includes(sort.tag)
                                        ) ?? -1;
