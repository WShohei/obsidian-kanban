...
export function astToUnhydratedBoard(
    ...
    root.children.forEach((child, index) => {
        if (child.type === 'heading') {
            ...
            let shouldApplyStatus: string | undefined;
            ...
            const list = getNextOfType(root.children, index, 'list', (child) => {
                ...
                if (child.type === 'paragraph') {
                    ...
                    const statusMatch = childStr.match(/^%\s*status:([^,\s]+)\s*%$/);
                    if (statusMatch) {
                        const statusValue = statusMatch[1];
                        if (statusValue === 'completion') {
                            shouldMarkItemsComplete = true;
                        } else {
                            shouldApplyStatus = statusValue;
                        }
                        return true;
                    }
                }
...
function laneToMd(lane: Lane) {
    ...
    if (lane.data.shouldApplyStatus && lane.data.shouldApplyStatus !== 'completion') {
        lines.push(`% status:${lane.data.shouldApplyStatus} %`);
        lines.push('');
    }
    ...
    if (lane.children.length === 0 || lines[lines.length - 1] !== '') {
        lines.push('');
    }
